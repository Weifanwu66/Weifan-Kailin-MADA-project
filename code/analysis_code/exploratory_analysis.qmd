...
ggplot(training_data_final %>% 
  ...
  geom_col() + 
  labs(x = "Pathogen Type", y = "Hospitalizations", title = "Stacked Bar Chart of Hospitalizations by \nPathogen Type and Etiology")
...
ggplot(training_data_final %>% 
         select(Pathogen_Type, Simplified_Etiology, Hospitalizations, `IFSAC Category`) %>% filter(Pathogen_Type == "Bacteria"), aes(x = Simplified_Etiology,y=`Hospitalizations`))+ 
  geom_col() + 
...
Location_v_Hospitalization <- ggplot(training_data_final, aes(x = Location, y = Hospitalizations)) +
  geom_col() + 
  labs(x = "Locations", y = "Hospitalizations", title = "Hospitalizations by Food Type, Excluding 'Multiple' and \n'Other'")+
  coord_flip()
...
ggplot(testing_data_final %>% group_by(Pathogen_Type, Simplified_Etiology, Illnesses) %>% filter(Pathogen_Type == "Bacteria"), aes(x = Simplified_Etiology)) + geom_bar() + labs(x = "Bacteria", y = "Cases", title = "Bar Chart of Bacteria Frequency")+
  coord_flip()
...
state_v_illness_testing <- testing_data_final %>% group_by(State) %>% mutate(all_illnesses = sum(Illnesses)) %>% count(all_illnesses) %>% select(State, all_illnesses) %>% arrange(desc(all_illnesses))

saveRDS(state_v_illness_testing, file = table_path)

rmarkdown::paged_table(state_v_illness_analysis)

# Food Type and Illnesses
ggplot(testing_data_final %>% filter(`IFSAC Category` != "Multiple") %>% filter(`IFSAC Category` != "Other"), aes(x = `IFSAC Category`, y = Illnesses)) + geom_col() + labs(x = "Food Type", y = "Illnesses", title = "Illnesses by Food Type, Excluding 'Multiple' and 'Other'")+
  coord_flip()
...
ggplot(testing_data_final, aes(x = Location, y = Illnesses)) +
  geom_col() + 
